...
仓库根目录可双击 `启动机器人管理.cmd`。选择遥操作组后，管理器可以直接启动 Dry-run 或真实运动组会话，并可停止当前组。组启动器 `run_group_from_registry.ps1` 会从 `robots.ini` 读取每台机器人的 IP 和伺服参数；未选中的机器人不会启动。运行期间每个 worker 会周期性写入 `status/<robot-id>.status`，管理界面显示模式、连接、上电、使能、拖动、伺服、故障、数据年龄、序列号、登录返回码、Servo 错误码、数据率、丢包数、watchdog 次数和读写错误。
